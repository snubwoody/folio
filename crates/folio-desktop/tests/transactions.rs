...
use folio_lib::{setup_test_db, Money};
...
async fn edit_transaction(pool: sqlx::SqlitePool) -> folio_lib::Result<()> {
    let conn = setup_test_db(pool.connect_options().get_filename()).await;
    let account = Account::create("", Money::ZERO, &conn)?;
    let a2 = Account::create("", Money::ZERO, &conn)?;
    let a3 = Account::create("", Money::ZERO, &conn)?;
    let date = NaiveDate::from_str("2024-12-12")?;
    let expense = Transaction::expense()
        .account_id(&account.id)
        .amount(Money::MAX)
...
async fn create_transfer(pool: sqlx::SqlitePool) -> folio_lib::Result<()> {
    let conn = setup_test_db(pool.connect_options().get_filename()).await;
    let a1 = Account::create("", Money::ZERO, &conn)?;
    let a2 = Account::create("", Money::ZERO, &conn)?;
    let date = NaiveDate::from_str("2024-12-12")?;
...
async fn set_account_for_transfer(pool: SqlitePool) -> folio_lib::Result<()> {
    ...
    let account = Account::create("__", Money::ZERO, &conn)?;
    let account2 = Account::create("__", Money::ZERO, &conn)?;
    let account3 = Account::create("__", Money::ZERO, &conn)?;
