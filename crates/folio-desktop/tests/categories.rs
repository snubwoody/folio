...
async fn soft_delete_category(pool: SqlitePool) -> Result<()> {
    ...
    let c = Category::fetch(&category.id, &conn)?;
...
async fn create_budgets_for_categories(pool: SqlitePool) -> Result<()> {
    let conn = setup_test_db(pool.connect_options().get_filename()).await;
    let c = Category::create_raw("__", &conn)?;
    let result = Budget::from_category(&c.id, &conn);
    assert!(result.is_err());
    create_missing_budgets(&conn)?;
    let result = Budget::from_category(&c.id, &conn);
    assert!(result.is_ok());
    Ok(())
}
...
async fn fetch_categories_not_deleted(pool: SqlitePool) -> crate::Result<()> {
    let conn = setup_test_db(pool.connect_options().get_filename()).await;
    let len1 = Category::fetch_all(&conn)?.len();
    Category::create("title", &conn)?;
    let c = Category::create("title", &conn)?;
    Category::delete(&c.id, &conn)?;
    let len2 = Category::fetch_all(&conn)?.len();
