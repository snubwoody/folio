...
use qmetaobject::{QAbstractTableModel,QGadget, QObject, qt_base_class, qt_method, qt_property};
use qttypes::{QByteArray, QModelIndex, QString, QVariant, QVariantMap};
...
impl From<EditAccountOpts> for EditAccount{
    fn from(value: EditAccountOpts) -> Self {
        ...
        let balance = value.starting_balance.and_then(|v|Money::from_str(&v.to_string()).ok());
        Self{
            name,
