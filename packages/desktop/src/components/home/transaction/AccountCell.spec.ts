...
test("Handle missing account", async () => {
    ...
    await expect
        .element(screen.getByRole("button", { name: "Select an item" }))
        .toBeInTheDocument();
});
...
test("Disable payee select items for expense", async () => {
    ...
    await expect
        .element(screen.getByRole("option", { name: "Payee" }))
        .toHaveAttribute("data-disabled", "");
    await expect
        .element(screen.getByRole("option", { name: "Account 2" }))
...
test("Display expense account if transfer", async () => {
    ...
    const account2 = await accountStore.createTestAccount({
