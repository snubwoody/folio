...
test("Value updates with date field", async () => {
    ...
    await expect
        ...
        .toHaveTextContent("January 2025");
});
...
test("Next month button increments month", async () => {
    const screen = await render(Calendar, {
        value: new CalendarDate(2020, 1, 1),
    });
    await expect
        .element(screen.getByRole("heading"))
    ...
    await expect
        .element(screen.getByRole("heading"))
        .toHaveTextContent("February 2020");
});

test("Previous month button decrements month", async () => {
    const screen = await render(Calendar, {
        value: new CalendarDate(2020, 10, 1),
    });
    await expect
        .element(screen.getByRole("heading"))
        .toHaveTextContent("October 2020");
    await screen.getByRole("button", { name: "Previous" }).click();
    await expect
        .element(screen.getByRole("heading"))
        .toHaveTextContent("September 2020");
});
