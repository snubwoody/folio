...
impl Account{
	pub async fn create(name: &str, starting_balance: Decimal,pool: &sqlx::SqlitePool){
		...
		sqlx::query!("INSERT INTO accounts(name,starting_balance) VALUES($1,$2)",name,balance)
			.execute(pool)
			.await
			.unwrap();
	}
...
mod test{
	...
	async fn fetch_account(pool:sqlx::SqlitePool){
		let record = sqlx::query!("INSERT INTO accounts(name,starting_balance) VALUES('C3PO','20.000') RETURNING id")
			.fetch_one(&pool)
			.await
			.unwrap();
